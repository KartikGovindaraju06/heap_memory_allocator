...
static const size_t reg_min_block_size = 2 * dsize;

/** @brief miniblock size (bytes) */
static const size_t mini_min_block_size = dsize;

/** @brief miniblock payload size */
static const size_t mini_payload_size = wsize;
...
/**
 * returns the second to last bit in header
 */
static const word_t prev_alloc_mask = 0x2;

/**
 * returns the third to last bit in header
 */
 ...
 */
static word_t pack(size_t size, bool alloc, bool prev_alloc, bool is_prev_mini) {
    word_t word = size;
    if (alloc) {
        word |= alloc_mask;
    }
...
static bool is_miniblock(block_t *block) {
    return get_size(block) == mini_min_block_size;
}
 ...
 */
static void write_block(block_t *block, size_t size, bool alloc, bool prev_alloc, bool is_prev_mini) {
    dbg_requires(block != NULL);
    dbg_requires(size > 0);
    block->header = pack(size, alloc, prev_alloc, is_prev_mini);

    // the footer is written only in free non-mini blocks
    ...
    // adjust the prev_alloc and prev_miniblock bit in the next block's header
    if (get_size(next) != 0) {
        if (alloc) {
...
static bool is_prev_miniblock(block_t *block) {
    return (block->header & prev_miniblock_mask);
}
...
static block_t *find_prev(block_t *block) {
    dbg_requires(block != NULL);
    dbg_requires(!get_prev_alloc(block)); // since footer exists only in free blocks

    if (is_prev_miniblock(block)) {
        return (block_t *)((char *)block - mini_min_block_size);
    }
...
static size_t get_index_from_asize(size_t asize) {
    ...
    dbg_assert((result != 0) && "non-miniblock was sent to bucket 0 of seglists");
    return result;
}
...
static block_t *coalesce_block(block_t *block) {
    ...
    if (!prev_alloc && !next_alloc) {
        ...
        write_block(prev, block_size, false, get_prev_alloc(prev), get_prev_mini(prev));
    ...
    if (prev_alloc && !next_alloc) {
        ...
        write_block(block, block_size, false, true, get_prev_mini(block));
        insert_to_freelist(block);
        return block;
    }
...
static void split_block(block_t *block, size_t asize) {
    ...
    size_t block_size = get_size(block);
    size_t remainder_block_size = block_size - asize;

    if ((remainder_block_size == mini_min_block_size) || (remainder_block_size >= reg_min_block_size)) {
        block_t *block_next;
        write_block(block, asize, true, get_prev_alloc(block), get_prev_mini(block));

        block_next = find_next(block);
        write_block(block_next, remainder_block_size, false, true, is_miniblock(block));

        insert_to_freelist(block_next);
    }
...
static block_t *find_fit(size_t asize) {
    ...
    // Take from top of miniblock freelist stack if asize == 16
    if ((asize == mini_min_block_size) && (seglists[index] != NULL)) {
        return seglists[index];
    }
 ...
 */
static bool check_starting_block(block_t *block) {
    return (get_size(block) == dsize) && get_alloc(block);
}
 ...
 * respectively point back to the current free block
 */
...
bool mm_checkheap(int line) {
    ...
    for (block = heap_start; get_size(block) > 0; block = find_next(block)) {
        ...
        if (get_size(block) < reg_min_block_size) {
...
void *malloc(size_t size) {
    ...
    // Allocate miniblock if size <= 8;
    if (size <= mini_payload_size) {
        asize = mini_min_block_size;
    }
    else {
        // Adjust block size to include overhead and to meet alignment requirements
        asize = round_up(size + wsize, dsize); 
        asize = max(asize, reg_min_block_size);
    }
